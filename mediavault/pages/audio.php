	...
	<div id="allMedia">
		...
		<?php
				...
				while($row=mysqli_fetch_array($result)){
					if((($row["mediavariety"] == 'mp3') || ($row["mediavariety"] == 'wav') || ($row["mediavariety"] == 'm4a') || ($row["mediavariety"] == 'wma') && ($row["filename"] != "")) && (trim($row["userID"]) == trim($thisUser))){
						$counter++;
					}
				}
				echo ($counter);
				...
				$result = mysqli_query($dbcon, $sql)or die(mysqli_error($dbcon));
				while($row=mysqli_fetch_array($result)){
					if((($row["mediavariety"] == 'mp3') || ($row["mediavariety"] == 'wav') || ($row["mediavariety"] == 'm4a') || ($row["mediavariety"] == 'wma')) && (trim($row["userID"]) == trim($thisUser))){

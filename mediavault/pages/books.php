	...
	<div id="allMedia">
		...
		<?php
				...
				while($row=mysqli_fetch_array($result)){
					if((($row["mediavariety"] == 'pdf') || ($row["mediavariety"] == 'epub') || ($row["mediavariety"] == 'mobi') && ($row["filename"] != "")) && (trim($row["userID"]) == trim($thisUser))){
						$counter++;
					}
				}
				echo ($counter);
				...
				$result = mysqli_query($dbcon, $sql)or die(mysqli_error($dbcon));
				while($row=mysqli_fetch_array($result)){
					if((($row["mediavariety"] == 'pdf') || ($row["mediavariety"] == 'epub') || ($row["mediavariety"] == 'mobi')) && (trim($row["userID"]) == trim($thisUser))){
